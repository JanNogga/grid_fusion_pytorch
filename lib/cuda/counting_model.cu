...
__global__ void counting_model_bayes_free_function_gpu(
    ...
    const torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> RangeMin,
    const torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits> RangeMax,
    const torch::PackedTensorAccessor32<float,4,torch::RestrictPtrTraits> Semseg,
    //output
    torch::PackedTensorAccessor32<float,5,torch::RestrictPtrTraits> out
    ) {
...
torch::Tensor counting_model_bayes_free_function(const torch::Tensor Grids, const torch::Tensor Origs, const torch::Tensor Dirs, const torch::Tensor Dists, const torch::Tensor RangeMin, const torch::Tensor RangeMax, const torch::Tensor Semseg, const int n_steps){
    ...
    CHECK(RangeMin.is_cuda()) << "RangeMin should be in GPU memory! Please call .cuda() on the tensor.";
    CHECK(RangeMax.is_cuda()) << "RangeMax should be in GPU memory! Please call .cuda() on the tensor.";
    CHECK(Semseg.is_cuda()) << "Semseg should be in GPU memory! Please call .cuda() on the tensor.";
    ...
    counting_model_bayes_free_function_gpu<<<blocks, BLOCK_SIZE>>>(
        ...
        RangeMin.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
        RangeMax.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
        Semseg.packed_accessor32<float,4,torch::RestrictPtrTraits>(),
        out.packed_accessor32<float,5,torch::RestrictPtrTraits>()
    );
    return out;
}